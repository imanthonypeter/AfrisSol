...
export function PerfilScreen() {
  ...
  const [name, setName] = useState("João Macuácua");
  const [phone, setPhone] = useState("+258 84 XXX XXXX");
    ...
    <div className="h-full flex flex-col overflow-y-auto" style={{ background: "#F5F7FA" }}>
        ...
        <div className="flex items-center gap-4">
          ...
          <div className="flex-1">
            {editMode ? (
              ...
              <p className="text-white" style={{ fontWeight: 700, fontSize: "17px" }}>{name}</p>
            )}
      ...
      <div className="px-5 -mt-4 mb-4">
        <div className="bg-white rounded-2xl p-4" style={{ boxShadow: "0 4px 20px rgba(0,0,0,0.06)" }}>
          ...
          {[
            { icon: <Phone size={15} color="#6366f1" />, label: phone, editable: true, setter: setPhone, value: phone },
            { icon: <Mail size={15} color="#F47C20" />, label: email, editable: true, setter: setEmail, value: email },
            { icon: <MapPin size={15} color="#22c55e" />, label: location, editable: true, setter: setLocation, value: location },
          ].map((info, i) => (
